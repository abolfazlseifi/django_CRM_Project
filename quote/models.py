...
minlenvalida = validators.MinValueValidator(limit_value=0,message=("discount can't be negative"))

maxlengthvalid = validators.MaxValueValidator(limit_value=100,message=("discount can't be more than 100"))
...
class QuoteFollowUp(models.Model):
    ...
    timestamp = models.DateTimeField(auto_now_add=True, verbose_name='تاریخ ثبت')
    creator = models.ForeignKey(get_user_model(), verbose_name='کاربر ثبت کننده', on_delete=models.CASCADE)
    content = models.TextField(verbose_name='متن پیگیری',max_length=400,default=None)
